...
class forumCustomRouter extends Router {
	public function parseRoute($request) {
		Debugger::getInstance()->message("Router (forum) parsed URI ".$request);
		...
		switch (count($_result)) {
			...
			case 4:
				$module = $_result[1]; $view   = $_result[2];
				$alias=$_result[3];
				$psid = Module::getHelper("posts","forum",true)->getIdByAlias($view,$alias);
