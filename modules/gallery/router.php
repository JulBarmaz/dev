...
class galleryCustomRouter extends Router {
	public function parseRoute($request) {
		Debugger::getInstance()->message("Router (gallery) parsed URI ".$request);
		...
		switch (count($_result)) {
			...
			case 4:
				...
				$psid = Module::getHelper("gallery","gallery",true)->getIdByAlias($view,$alias);
	...
	public function buildRoute($options, $mode="sitelink", $force_fronte=0, $absolute_link=0, $force_protocol=0) {
		...
		if (array_key_exists('multy_code', $options)) unset($options['multy_code']);
