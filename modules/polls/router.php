...
class pollsCustomRouter extends Router {
	public function parseRoute($request) {
		Debugger::getInstance()->message("Router (polls) parsed URI ".$request);
		...
		switch (count($_result)) {
			...
			case 4:
				$module = $_result[1]; 	$view   = $_result[2];
				$alias=$_result[3];
				$psid = Module::getHelper("polls","polls",true)->getIdByAlias($view,$alias);
